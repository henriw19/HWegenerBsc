...
# get data:
parameters = ['[[31,1,7]]','[[61,1,9]]','[[91,1,11]]','[[127,1,13]]','[[169,1,15]]']
colors = ['cornflowerblue','khaki','plum','mediumseagreen','lightsteelblue','moccasin', 'yellowgreen','salmon']
skiplist = [4,7,10,13,5,1,1,1]

fig, ax = plt.subplots()
for distance in range(10,11,2):
    data = np.loadtxt(f"thresholddata/tricoloruniondistance{distance}data.txt", delimiter=',', skiprows=1)
    p = data[:, 0]
    log = data[:, 1]
    ax.plot(p,log, label = distance)
...
ax.set_ylabel('y')
ax.legend()
plt.axvline(x=0.046, color='0', linestyle='--')
...
plt.legend(loc = 0)
plt.title('Hexagonal Planar Color Code Threshold')
